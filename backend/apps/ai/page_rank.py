...
class YoutubeInference:
	...
	def get_transcript(self, lang: str = "ko") -> ty.Dict[str, dict]:
		...
		scripts: dict = YouTubeTranscriptApi.get_transcript(youtube_id, languages={lang})
	...
	def get_script_keywords(self, scripts):
		keywords = summarize_with_keywords(texts=scripts, min_count=self.min_count, max_length=self.max_length,
										   beta=self.beta, max_iter=self.max_iter, stopwords=self.stop_words, verbose=self.verbose)
		keywords = summarize_with_keywords(scripts)  # with default arguments

		words = []
		word_importance = {}
		for word, importance in sorted(keywords.items(), key=lambda x:x[1], reverse=True):
			#print('%8s:\t%.4f' % (word, r))
			words.append(word)

		for word, importance in sorted(keywords.items(), key=lambda x:x[1], reverse=True)[:20]:
			#print('%8s:\t%.4f' % (word, r))
	...
	def preprocessing(self, script):
		...
		new_script = re.sub('[-=+,#/\?:^$.@*\"※~&%ㆍ!』\\‘|\(\)\[\]\<\>`\'…》]', '', new_script)
		return new_script
